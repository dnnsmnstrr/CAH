...
class ComplicationController: NSObject, CLKComplicationDataSource {
    ...
    func getLocalizableSampleTemplate(for complication: CLKComplication, withHandler handler: @escaping (CLKComplicationTemplate?) -> Void) {
        if complication.family == .utilitarianSmall {
            ...
            smallFlat.textProvider = CLKSimpleTextProvider(text: "Cards.")
            smallFlat.imageProvider = CLKImageProvider(onePieceImage: UIImage(named: "Logo")!)
            handler(smallFlat)
        }
        else if complication.family == .utilitarianLarge {
            let largeFlat = CLKComplicationTemplateUtilitarianLargeFlat()
            largeFlat.textProvider = CLKSimpleTextProvider(
                text: "New Card!", shortText:"Cards.")
            largeFlat.imageProvider = CLKImageProvider(
                onePieceImage: UIImage(named: "Logo")!)
            handler(largeFlat)
        }
